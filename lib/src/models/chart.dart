...
class CycleStats {
  ...
  static const CycleStats empty =
      CycleStats(mucusCycleScore: null, lengthOfPostPeakPhase: null);

  const CycleStats(
      {required this.mucusCycleScore, required this.lengthOfPostPeakPhase});

  CycleStats setMucusCycleScore(double? score) {
    return CycleStats(
        mucusCycleScore: score, lengthOfPostPeakPhase: lengthOfPostPeakPhase);
  }

  CycleStats setLengthOfPostPeakPhase(int? length) {
    return CycleStats(
        mucusCycleScore: mucusCycleScore, lengthOfPostPeakPhase: length);
  }

  factory CycleStats.fromJson(Map<String, dynamic> json) =>
      _$CycleStatsFromJson(json);
...
class ChartEntry {
  ...
  ChartEntry(
      ...
      this.renderedObservation,
      this.manualSticker});

  static ChartEntry fromRenderedObservation(
