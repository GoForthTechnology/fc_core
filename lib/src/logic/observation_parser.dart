...
DischargeSummary _getDischargeSummary(String input) {
  ...
  DischargeDescriptor? descriptorRequiringColor =
      dischargeDescriptors.where((d) => d.requiresColor).firstOrNull;
  ...
  if (input.isNotEmpty) {
    throw Exception(
        "Nothing should follow discharge summary. Extra text: $input");
  }
...
String _consumeDescriptor(String input, List<DischargeDescriptor> descriptors) {
  for (var descriptor in DischargeDescriptor.values) {
    if (input.startsWith(descriptor.code)) {
      ...
      return _consumeDescriptor(
          consumePrefix(descriptor.code, input), descriptors);
    }
  }
  return input;
}
