...
List<RenderedObservation> renderObservations(List<Observation> observations,
  ...
  for (int i = 0; i < observations.length; i++) {
    ...
    var shouldAskESQ = canAskESQ &&
        daysOfMucus > 1 &&
        !(countsOfThree.getCount(CountOfThreeReason.peakDay, i) > 3);
    var isPointOfChange = shouldAskESQ && /*yesterdayWasEssentiallyTheSame &&*/
        !(observation.essentiallyTheSame ?? false);
    if (isPointOfChange) {
    ...
    if (observation.hasNonPeakTypeMucus) {
      ...
      if (!observation.hasMucus &&
        ...
        countsOfThree.registerCountStart(
            CountOfThreeReason.consecutiveDaysOfNonPeakMucus, i - 1);
      }
    ...
    if (observation.hasPeakTypeMucus) {
      countsOfThree.registerCountStart(
          CountOfThreeReason.singleDayOfPeakMucus, i);
    }
    ...
    if (observation.hasMucus ||
        countsOfThree.inCountOfThree(CountOfThreeReason.peakDay, i)) {
    ...
    if (!isPostPeak &&
        (consecutiveDaysOfNonPeakMucus >= 3 ||
            countsOfThree.inCountOfThree(
                CountOfThreeReason.consecutiveDaysOfNonPeakMucus, i))) {
    ...
    if (hasUnusualBleeding ||
        countsOfThree.inCountOfThree(CountOfThreeReason.unusualBleeding, i)) {
      fertilityReasons.add(Instruction.d6);
    }

    bool hasAnotherEntry = i + 1 < observations.length;
    bool isPeakDay = !isPointOfChange &&
        hasAnotherEntry &&
        (consecutiveDaysOfPeakMucus > 0 &&
            !observations[i + 1].hasPeakTypeMucus);
    if (isPeakDay) {
    ...
    if (prePeakYellowEnabled && !isPostPeak && inEssentialSamenessPattern) {
      ...
      countsOfThree
          .clearCount(CountOfThreeReason.consecutiveDaysOfNonPeakMucus);
